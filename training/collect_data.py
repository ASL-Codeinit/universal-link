...
TARGET_COUNT =20 #for future datasets, fewer frames per sign + more variations of each sign apt for LSTM
...
signer_id = input("\n>>> Enter Signer ID: ").strip().lower()
print(f"Signer selected: {signer_id}")

# --- THE WORD LOOP BEGINS ---
previous_label = None
while True:
    if previous_label:
        prompt = f"\n>>> Enter Word (or 'exit' to stop recording) [Press Enter to repeat '{previous_label}']: "
    else:
        prompt = "\n>>> Enter Word (or 'exit' to stop recording): "
    
    user_input = input(prompt).strip().lower()
    
    # If user presses Enter with no input, use previous label
    if not user_input:
